...
**Link:** `https://<your-app>.onrender.com/` *(replace with your deployed URL — no fixed production URL is set in the code; the app defaults to `http://127.0.0.1:8000/` for local use)*
...
![Home / sign-in screen](images/home.png)
...
![Sign in with X](images/signin.png)
...
![X Ads Account section with detected chips and Active badge](images/ads-account.png)
...
![Current card details loaded](images/current-card.png)
...
![Media picker — Browse library and Upload tabs](images/media-picker.png)
...
![New title and destination URL fields](images/new-values.png)
...
![Scheduling controls](images/schedule-time.png)
...
![Scheduled updates list with statuses and actions](images/schedules-list.png)
...
| **Required** badge won't turn green | Enter a valid X Ads **Account ID** in the field, or click a detected chip. Confirm your X account has an active Ads account at [ads.x.com](https://ads.x.com) → Account settings, and that you're signed in as that user. |
...
| Media library is empty | Your Ads account has no media of the required type. Upload assets at [ads.x.com](https://ads.x.com) first, or use the **Upload** tab to add a new file. |
